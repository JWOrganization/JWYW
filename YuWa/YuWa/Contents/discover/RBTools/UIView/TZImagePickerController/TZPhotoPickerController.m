...
@property (nonatomic,strong)JWImgPickerAlbumChooseView * naviIMGTittleView;
@end
...
- (void)viewDidLoad {
    ...
    UIView * lineView = [[UIView alloc]initWithFrame:CGRectMake(0.f, self.navigationController.navigationBar.height - 1.f, [UIScreen mainScreen].bounds.size.width, 1.f)];
    lineView.backgroundColor = [UIColor lightGrayColor];
    [self.navigationController.navigationBar addSubview:lineView];
    
    self.navigationItem.title = _model.name;
    self.navigationItem.leftBarButtonItem = [UIBarButtonItem barItemWithImageName:nil withSelectImage:nil withHorizontalAlignment:UIControlContentHorizontalAlignmentLeft withTittle:@"取消" withTittleColor:[UIColor lightGrayColor] withTarget:self action:@selector(cancel) forControlEvents:UIControlEventTouchUpInside withWidth:40.f];
    self.navigationItem.rightBarButtonItem = [UIBarButtonItem barItemWithImageName:nil withSelectImage:nil withHorizontalAlignment:UIControlContentHorizontalAlignmentCenter withTittle:@"继续" withTittleColor:[UIColor lightGrayColor] withTarget:self action:@selector(okButtonClick) forControlEvents:UIControlEventTouchUpInside withWidth:40.f];
    ...
    [self imageArrGet];//获取所有照片数组
    // [self resetCachedAssets];
    self.naviIMGTittleView = [[[NSBundle mainBundle]loadNibNamed:@"JWImgPickerAlbumChooseView" owner:nil options:nil]firstObject];
    self.naviIMGTittleView.dataArr = [self.albumArr mutableCopy];
    WEAKSELF;
    self.naviIMGTittleView.chooseTypeBlock = ^(NSString * index){
        TZAlbumModel *albumModel = weakSelf.albumArr[[index integerValue]];
        _models = [NSMutableArray arrayWithArray:albumModel.models];
        [weakSelf.collectionView reloadData];
    };
    self.navigationItem.titleView = self.naviIMGTittleView;
}

- (void)makeRightBarBtnWithCount:(NSString *)count{
    if ([count isEqualToString:@"0"]) {
        self.navigationItem.rightBarButtonItem = [UIBarButtonItem barItemWithImageName:nil withSelectImage:nil withHorizontalAlignment:UIControlContentHorizontalAlignmentCenter withTittle:@"继续" withTittleColor:[UIColor lightGrayColor] withTarget:self action:@selector(okButtonClick) forControlEvents:UIControlEventTouchUpInside withWidth:40.f];
    }else{
        self.navigationItem.rightBarButtonItem = [UIBarButtonItem barItemWithImageName:nil withSelectImage:nil withHorizontalAlignment:UIControlContentHorizontalAlignmentCenter withTittle:[NSString stringWithFormat:@"继续(%@)",count] withTittleColor:[UIColor colorWithHexString:@"#48A50F"] withTarget:self action:@selector(okButtonClick) forControlEvents:UIControlEventTouchUpInside withWidth:50.f];
    }
...
- (void)refreshBottomToolBarStatus {
    ...
    [self makeRightBarBtnWithCount:[NSString stringWithFormat:@"%zd",imagePickerVc.selectedModels.count]];
...
- (void)imageArrGet{
    ...
    [[TZImageManager manager] getAllAlbums:imagePickerVc.allowPickingVideo allowPickingImage:imagePickerVc.allowPickingImage completion:^(NSArray<TZAlbumModel *> *models) {
        ...
        for (TZAlbumModel *albumModel in self.albumArr) {
            if ([albumModel.name isEqualToString:@"相机胶卷"] || [albumModel.name isEqualToString:@"Camera Roll"] ||  [albumModel.name isEqualToString:@"所有照片"] || [albumModel.name isEqualToString:@"All Photos"]) {
